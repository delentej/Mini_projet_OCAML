...
let export path (graph: string graph) = 
  ...
  let rec loop = function
    |[] -> fprintf ff " ;\n"
    |a::rest -> fprintf ff " %s " a ; loop rest
  in
  loop (Graph.list_id graph);
  
  fprintf ff "        node [shape = circle]; \n";

(*  v_iter graph (fun id out ->
      fprintf ff "%s -> " id  ;
      List.iter(fun (id2, lbl) ->  fprintf ff " %s [ label = \" %s \" ];\n\n "  id2 lbl ) out ); *)

  v_iter graph (fun id ((id2,cout)::rest) -> fprintf ff "%s" id ; fprintf ff " ->  %s" id2 ; fprintf ff "[ label = \" %s \" ]; \n" cout ) ;
